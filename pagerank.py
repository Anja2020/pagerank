import os
import random
import re
import sys

DAMPING = 0.85
SAMPLES = 10000


def main():
    if len(sys.argv) != 2:
        sys.exit("Usage: python pagerank.py corpus")
    corpus = crawl(sys.argv[1])
    ranks = sample_pagerank(corpus, DAMPING, SAMPLES)
    print(f"PageRank Results from Sampling (n = {SAMPLES})")
    for page in sorted(ranks):
        print(f"  {page}: {ranks[page]:.4f}")
    ranks = iterate_pagerank(corpus, DAMPING)
    print(f"PageRank Results from Iteration")
    for page in sorted(ranks):
        print(f"  {page}: {ranks[page]:.4f}")


def crawl(directory):
    """
    Parse a directory of HTML pages and check for links to other pages.
    Return a dictionary where each key is a page, and values are
    a list of all other pages in the corpus that are linked to by the page.
    """
    pages = dict()

    # Extract all links from HTML files
    for filename in os.listdir(directory):
        if not filename.endswith(".html"):
            continue
        with open(os.path.join(directory, filename)) as f:
            contents = f.read()
            links = re.findall(r"<a\s+(?:[^>]*?)href=\"([^\"]*)\"", contents)
            pages[filename] = set(links) - {filename}

    # Only include links to other pages in the corpus
    for filename in pages:
        pages[filename] = set(
            link for link in pages[filename]
            if link in pages
        )

    return pages


def transition_model(corpus, page, damping_factor):
    """
    Return a probability distribution over which page to visit next,
    given a current page.

    With probability `damping_factor`, choose a link at random
    linked to by `page`. With probability `1 - damping_factor`, choose
    a link at random chosen from all pages in the corpus.
    """
    distribution = {}
    links = list(corpus[page])

    if len(links) > 0:
        # equal probability for each page in the corpus
        equal_prop = (1 - damping_factor) / len(corpus)

        # propability for links of the page
        links_prob = damping_factor / len(links)

        # set probability for each page
        for page in corpus:
            distribution[page] = equal_prop
            if page in links:
                distribution[page] += links_prob

    else:
        prop = 1 / len(corpus)
        for page in corpus:
            distribution[page] = prop

    return distribution


def sample_pagerank(corpus, damping_factor, n):
    """
    Return PageRank values for each page by sampling `n` pages
    according to transition model, starting with a page at random.

    Return a dictionary where keys are page names, and values are
    their estimated PageRank value (a value between 0 and 1). All
    PageRank values should sum to 1.
    """
    pageRank = {}
    pages = list(corpus.keys())
    samples = []

    # choose first page randomly and add sample
    page = random.choice(pages)
    samples.append(page)

    # sample n times
    for i in range(n):
        # get probability distribution for previous page
        distribution = transition_model(corpus, page, damping_factor)
        # choose next page to be visited
        next_pages = []
        for page in distribution.keys():
            next_pages.append(page)
        page = random.choices(next_pages, list(distribution.values()), k=1)[0]
        samples.append(page)

    # calculate PageRank for each page
    for page in pages:
        pageRank[page] = samples.count(page) / n

    return pageRank


def iterate_pagerank(corpus, damping_factor):
    """
    Return PageRank values for each page by iteratively updating
    PageRank values until convergence.

    Return a dictionary where keys are page names, and values are
    their estimated PageRank value (a value between 0 and 1). All
    PageRank values should sum to 1.
    """
    pageRank = {}
    repeat = True

    # initialize pageRank for each page with 1 / N
    for page in corpus.keys():
        pageRank[page] = 1 / len(corpus)

    # calulate probability to randomly choose the page
    random_prob = (1 - damping_factor) / len(corpus)

    # calculate until no PageRank value changes by more than 0.001 between the current rank values and the new rank values.
    while repeat:
        current_pr = pageRank.copy()
        # iterate through pages in pageRank
        for page in pageRank:
            linking_prob = 0
            
            for link in corpus.keys():
                # chech if page is linked in the link
                if page in corpus[link]:
                    # add to sum of probability that the surfer followed a link to the current page
                    linking_prob += pageRank[link] / len(corpus[link])
            
            # caculate total PR of current page
            pageRank[page] = random_prob + damping_factor * linking_prob

        # check if PageRank value changes by more than 0.001 between the current rank values and the new rank values
        pr_delta = {key: pageRank[key]-current_pr[key] for key in pageRank}
        repeat = False
        for page in pr_delta:
            if pr_delta[page] <= -0.001 or pr_delta[page] >= 0.001:
                repeat = True

    return pageRank


if __name__ == "__main__":
    main()
